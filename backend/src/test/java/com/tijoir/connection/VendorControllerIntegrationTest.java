...
class VendorControllerIntegrationTest {
    ...
    @Test
    void offboardVendorWithSameIdempotencyKeyReturnsOriginalResponse() throws Exception {
        String ownerToken = registerVerifyAndLogin("Acme Vendor Replay", "owner@acme-vendor-replay.test");
        ...
        String vendorResponse = mockMvc.perform(post("/api/vendors")
                        ...
                        .content("""
                                {
                                  "name": "Replay Vendor",
                                  "contactName": "Replay Operator",
                                  "contactEmail": "ops@replay.test"
                                }
                                """))
                .andExpect(status().isCreated())
                .andReturn()
        ...
        String vendorId = objectMapper.readTree(vendorResponse).get("id").asText();
        String secretId = createSecret(ownerToken, "Replay Secret", "API_KEY", "ReplaySecret@123");
        String contractResponse = mockMvc.perform(post("/api/vendors/" + vendorId + "/contracts")
                        .header("Authorization", "Bearer " + ownerToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                        ...
                        .content("""
                                {
                                  ...
                                  "vendorId": "%s",
                                  "contractId": "%s",
                                  "recipientLabel": "Replay operator"
                                }
                                """.formatted(secretId, vendorId, contractId)))
                .andExpect(status().isCreated());

        String idempotencyKey = "vendor-offboard-key";
        String firstResponse = mockMvc.perform(post("/api/vendors/" + vendorId + "/offboard")
                        .header("Authorization", "Bearer " + ownerToken)
        ...
        String secondResponse = mockMvc.perform(post("/api/vendors/" + vendorId + "/offboard")
                        .header("Authorization", "Bearer " + ownerToken)
                        .header("Idempotency-Key", idempotencyKey))
                .andExpect(status().isOk())
                .andReturn()
        ...
        org.junit.jupiter.api.Assertions.assertEquals(first.get("vendorId").asText(), second.get("vendorId").asText());
        org.junit.jupiter.api.Assertions.assertEquals(first.get("revokedContracts").asInt(), second.get("revokedContracts").asInt());
        org.junit.jupiter.api.Assertions.assertEquals(first.get("revokedShareLinks").asInt(), second.get("revokedShareLinks").asInt());
    }
